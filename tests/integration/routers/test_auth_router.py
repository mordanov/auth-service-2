...
class TestLoginEndpoint:
    async def test_valid_credentials_sets_cookie(self, client, db):
        await make_user(db, username="login_ok", password_hash=_pwd.hash("pass123"))
        await db.commit()
    ...
    async def test_wrong_password_returns_401(self, client, db):
        await make_user(db, username="login_bad", password_hash=_pwd.hash("correct"))
        await db.commit()
    ...
    async def test_blocked_user_returns_403(self, client, db):
        await make_user(db, username="login_blocked", password_hash=_pwd.hash("pw"), is_active=False)
        await db.commit()
...
class TestLogoutEndpoint:
    async def test_logout_clears_cookie(self, client, db):
        user = await make_user(db, username="logout_user", password_hash=_pwd.hash("pw"))
        await db.commit()
