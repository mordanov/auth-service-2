...
For this project, treat `prompt/auth-middleware-prompt.md` as the business source of truth. Test plans and release recommendations must verify the documented authentication gateway behavior rather than generic auth-service assumptions.

- Verify **auth-app** end-to-end: username/password login, Google OAuth2 login, GitHub OAuth login, admin user management, per-app access toggle, token verification by protected apps, token expiry and cleanup, and RU/EN language switching.
- Verify OAuth rejection: any OAuth login with an email that does not match an existing `users` row must be denied with a clear error — no auto-registration.
- Verify the admin panel: admin can create users, block/unblock users, and toggle per-app access checkboxes for all 8 apps. Non-admin users must be blocked from all `/admin` endpoints.
- Verify `GET /api/verify-token`: valid token returns `{ user_id, username, role, apps }`; expired, invalid, or missing token returns 401. Test both cookie and `Authorization: Bearer` delivery.
- Verify that `auth_logs` records each login attempt (success and failure) with correct actor and timestamp.
- Verify httpOnly cookie attributes: `Secure`, `HttpOnly`, `SameSite=Lax`, and domain `.mainpage.com`.
- Verify token cleanup: tokens past `expires_at` are removed by the background job and rejected by verify endpoint.
- Verify the seed: on fresh DB startup, 3 users from `.env` are created with correct roles.
- System gate: `docker compose up --build` must start backend and frontend with no manual steps beyond `.env` setup; OpenAPI docs must be reachable at `/docs`.
