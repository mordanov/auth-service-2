...
For this project, treat `prompt/auth-middleware-prompt.md` as the business source of truth. Architecture decisions must preserve the documented product scope, API surface, technology stack, and success criteria unless a formal Product Manager-approved change updates all downstream artifacts.

- Architect **auth-app**, a centralized authentication gateway acting as the single "front door" for 8 existing family web applications: `budget-site`, `family-admin-routine`, `family-archive`, `family-kitchen-recipes`, `new-site`, `portuguese-expenses`, `reminders-app`, and `servinga-dashboard`.
- Use the specified stack as constraints: Python 3.12 + FastAPI backend, React.js + HeroUI frontend (default style, no customization), PostgreSQL shared from `web-folders`, Nginx from `web-folders`, Docker Compose following the same pattern as sibling applications, and RU/EN i18n.
- Design the opaque-token authentication model: random 64-character tokens stored in the `auth_tokens` PostgreSQL table (`token_value`, `user_id`, `expires_at`, `created_at`, `last_used_at`), delivered as an httpOnly cookie (`auth_token`) with `SameSite=Lax`, `Secure`, domain `.mainpage.com`, and TTL controlled by `TOKEN_TTL_HOURS` env var.
- Preserve the documented authentication methods: username/password (bcrypt), Google OAuth2, and GitHub OAuth — all OAuth logins must match existing DB accounts by email only; unknown users are denied and never auto-registered.
- Preserve the documented API surface: `GET /api/verify-token` returning `{ user_id, username, role, apps }` on 200 and `{ error: "unauthorized" }` on 401; admin endpoints `GET/POST /api/admin/users` and `GET/POST /api/admin/apps`; auth endpoints `/login`, `/logout`, `/callback/google`, `/callback/github`.
- Model domain entities exactly: `users` (UUID pk, username, email, password_hash, role, is_active, google_id, github_id, created_at), `user_app_access` (user_id FK, app_name, is_enabled), `auth_tokens` (token_value, user_id FK, expires_at, created_at, last_used_at), `auth_logs` (login attempts, success/failure, timestamp, actor).
- Role architecture: `admin` — access to `auth-app` only, creates users (no deletion, only blocking), manages per-user per-app access checkboxes, views user list. `user` — access only to permitted applications, no admin panel access.
- On first run, a seed/migration creates 3 users from `.env`: `ADMIN_USERNAME/ADMIN_PASSWORD`, `USER1_USERNAME/USER1_PASSWORD`, `USER2_USERNAME/USER2_PASSWORD`.
- Integration architecture: each protected app must remove its own auth logic and add middleware/decorator that calls `GET /api/verify-token` via httpOnly cookie or `Authorization: Bearer` header; unauthenticated requests redirect to `AUTH_APP_URL/login?redirect=<original-url>`.
- Nginx architecture: `auth.mainpage.com` proxies to `auth-app:8000`; each protected app config adds `proxy_cookie_domain auth.mainpage.com .mainpage.com` so the shared cookie is sent across subdomains.
- Background cleanup: expired tokens must be removed by a background task (APScheduler or FastAPI background tasks).
- Delivery gates must include Docker Compose startup, OpenAPI docs available at `/docs`, CORS configured for `*.mainpage.com`, all secrets via `.env` only, and security review for OAuth callback flows, token verification, admin privilege escalation, and cookie handling.
