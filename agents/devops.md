...
For this project, treat `prompt/auth-middleware-prompt.md` as the business source of truth. Platform work must support the documented stack and deployment topology rather than introducing alternate services, databases, or deployment assumptions without approval.

- Provide local orchestration with **Docker Compose** for `backend` and `frontend` containers; `docker compose up --build` must start the app with no manual steps beyond `.env` setup. The app connects to the **shared PostgreSQL** instance from `web-folders` — do not provision a separate database service in the auth-app compose file.
- Runtime stack assumptions: **Python 3.12 FastAPI backend**, **React.js + HeroUI frontend**, **PostgreSQL from `web-folders`**, **Nginx from `web-folders`**.
- Configure Nginx in `web-folders` for `auth.mainpage.com` proxying to `auth-app:8000`, and add `proxy_cookie_domain auth.mainpage.com .mainpage.com` to each of the 8 protected-app server blocks so the shared `.mainpage.com` cookie is forwarded correctly.
- Document and protect all required environment variables: `DATABASE_URL`, `ADMIN_USERNAME/PASSWORD`, `USER1_USERNAME/PASSWORD`, `USER2_USERNAME/PASSWORD`, `TOKEN_TTL_HOURS`, `SECRET_KEY`, `GOOGLE_CLIENT_ID/SECRET/REDIRECT_URI`, `GITHUB_CLIENT_ID/SECRET/REDIRECT_URI`, `AUTH_APP_URL`, `COOKIE_DOMAIN`.
- Secrets must never be committed, logged, or exposed through build artifacts. Provide `.env.example` with placeholder values.
- Operational readiness must include backend health/readiness checks, Alembic migration execution on startup, frontend static serving via Nginx, and clear rollback/rebuild instructions.
