...
For this project, treat `prompt/auth-middleware-prompt.md` as the business source of truth. Backend implementation must not substitute another product scope, authentication model, token strategy, or API shape without an explicit Product Manager and Software Architect decision.

- Build the backend for **auth-app** using **Python 3.12** and **FastAPI**, with SQLAlchemy async ORM (or equivalent), Alembic migrations, and shared **PostgreSQL** from `web-folders`.
- Implement all three authentication methods: username/password (bcrypt hash stored in `users.password_hash`), Google OAuth2 callback (`/callback/google`), and GitHub OAuth callback (`/callback/github`). OAuth logins must match existing DB accounts by email only — unknown emails are rejected; there is no auto-registration flow.
- Implement opaque token issuance: generate a cryptographically random 64-character token, persist to `auth_tokens` (`token_value`, `user_id`, `expires_at`, `created_at`, `last_used_at`), and deliver via httpOnly cookie `auth_token` with `SameSite=Lax`, `Secure`, domain from `COOKIE_DOMAIN` env var.
- Implement `GET /api/verify-token`: accept token from `Cookie: auth_token` or `Authorization: Bearer`; return `{ user_id, username, role, apps }` on 200, `{ error: "unauthorized" }` on 401. This endpoint is called by all protected applications.
- Implement admin endpoints: `GET /api/admin/users` (list all users with status), `POST /api/admin/users` (create user; no deletion, only `is_active` blocking), `GET/POST /api/admin/apps` (read/update per-user app access checkboxes for all 8 apps).
- Implement `/login`, `/logout`, `/callback/google`, `/callback/github` routers according to the documented structure.
- Implement DB seed (`db/seed.py`): on first run create admin + 2 users from `ADMIN_USERNAME/ADMIN_PASSWORD`, `USER1_USERNAME/USER1_PASSWORD`, `USER2_USERNAME/USER2_PASSWORD` env vars.
- Implement background token cleanup: delete expired tokens using APScheduler or FastAPI background tasks, controlled by `TOKEN_TTL_HOURS` (default 24h).
- Log every login attempt (success and failure) to `auth_logs` table with timestamp and actor identity.
- Configure CORS for `*.mainpage.com` domains. All secrets must come from `.env` — never hardcoded. Expose OpenAPI docs at `/docs`.
