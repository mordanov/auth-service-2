...
For this project, treat `prompt/auth-middleware-prompt.md` as the business source of truth. Frontend implementation must not substitute another product scope, UI flow, or authentication behavior without an explicit Product Manager and Software Architect decision.

- Build the UI for **auth-app** using **React.js** and **HeroUI** default components — no style customization beyond what HeroUI provides out of the box.
- Implement RU/EN internationalization with a language switcher in the top-right corner of every page. No user-facing labels may be hardcoded in JSX.
- Implement the **Login Page** (`/login`):
  - Three tabs: "Sign in with password" / "Google" / "GitHub".
  - Password tab: username/email + password fields.
  - OAuth tabs: provider icon buttons that initiate the OAuth redirect flow.
  - Display clear error messages for invalid credentials, unknown OAuth email, or blocked account.
- Implement the **Admin Panel** (`/admin`) — accessible only to users with role `admin`:
  - User table showing name, email, role, and status (active / blocked).
  - "Create user" button with a form (username, email, password, role).
  - "Block" button per row (no deletion).
  - Expandable row showing per-app access checkboxes for all 8 protected applications.
- Handle the `auth_token` httpOnly cookie automatically through the browser; never read, write, or copy it in JavaScript.
- Never rely on client-only checks for authorization — the admin panel and protected routes must be gated by the server-side role returned from `GET /api/verify-token`.
- Redirect unauthenticated users to `/login?redirect=<original-url>`.
- Handle loading, error, and permission-denied states for all pages and API calls.
