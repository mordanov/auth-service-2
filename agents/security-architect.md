...
For this project, treat `prompt/auth-middleware-prompt.md` as the business source of truth. Security guidance must protect the authentication gateway, OAuth flows, opaque token lifecycle, admin privilege model, and cross-domain cookie without replacing them with a different approach unless formally approved.

- Threat-model **auth-app** as a public-facing authentication gateway handling passwords (bcrypt), opaque tokens, OAuth2 authorization codes/callbacks (Google and GitHub), httpOnly cookies, admin user management, and cross-app token verification.
- Authentication controls must match the requirement: bcrypt-hashed passwords, opaque 64-character random tokens stored in PostgreSQL (never JWT), delivered as httpOnly `auth_token` cookie (`SameSite=Lax`, `Secure`, domain `.mainpage.com`), TTL from `TOKEN_TTL_HOURS`.
- OAuth security: Google and GitHub callbacks must verify state parameters (CSRF protection), exchange codes server-side only, and deny access for any email not already in the `users` table — no auto-registration under any circumstances.
- Admin privilege: the `admin` role must be enforced server-side on every admin endpoint; client-side role checks are supplementary only. Users may not promote their own role.
- Token verification endpoint `GET /api/verify-token` is called by all 8 protected apps — ensure it cannot be abused for enumeration, timing attacks, or information leakage beyond the documented response.
- Ensure all credentials (`GOOGLE_CLIENT_SECRET`, `GITHUB_CLIENT_SECRET`, database passwords, `SECRET_KEY`) are never hardcoded, logged, or exposed to the frontend bundle or error responses.
- CORS must be configured for `*.mainpage.com` only — no wildcard that admits other origins.
- Cookie security: `auth_token` must have `HttpOnly`, `Secure`, and `SameSite=Lax`; `COOKIE_DOMAIN` must come from `.env`; never expose the raw token value in JSON API responses.
- Define security tests for: unrecognized OAuth email rejected, expired/invalid/missing token rejected by verify endpoint, non-admin blocked from admin endpoints, OAuth state mismatch rejected, token replay after logout, brute-force password attempt logging, and CORS rejection for out-of-domain origins.
