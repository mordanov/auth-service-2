...
For this project, treat `prompt/auth-middleware-prompt.md` as the business source of truth. Product decisions must preserve the documented MVP unless a change is explicitly recorded with rationale, impact, and downstream updates to specs, plans, tasks, tests, and agent guidance.

- The product is **auth-app**, a centralized authentication gateway for 8 family web applications. The core value: a single login experience that controls access across all apps without duplicating authentication logic in each one.
- In-scope MVP: username/password login, Google OAuth2 login, GitHub OAuth login (all OAuth methods restricted to pre-existing accounts matched by email; no auto-registration), opaque token issuance via httpOnly cookie, token verification endpoint for protected apps, admin panel for user management and per-app access control, RU/EN language switcher, and Docker Compose startup.
- Role model: `admin` can create users (no deletion, only blocking), manage per-user access checkboxes for all 8 protected apps, and view the full user list. `user` can only access permitted applications.
- On first run, a seed/migration must create 3 users from environment variables: one admin and two regular users.
- Non-negotiable business rules: OAuth logins are denied for unrecognized emails; tokens expire after `TOKEN_TTL_HOURS` (default 24h); expired tokens are cleaned up by a background job; every login attempt (success and failure) is logged to `auth_logs`; all secrets are via `.env` only.
- Protected application list is fixed: `budget-site`, `family-admin-routine`, `family-archive`, `family-kitchen-recipes`, `new-site`, `portuguese-expenses`, `reminders-app`, `servinga-dashboard`.
- Success criteria: `docker compose up --build` starts the app; all three auth methods work; admin can create/block users and toggle app access; token verification returns correct user info to protected apps; `auth_logs` records attempts; OpenAPI docs available at `/docs`; CORS works for `*.mainpage.com`.
